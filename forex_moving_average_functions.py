...
def create_moving_average_tick ( pair, length1, length2, granularity, token ):
	...
	response = requests.get('https://api-fxtrade.oanda.com/v1/candles?instrument=' + pair + '&count=' + str(length1 + 1) + '&candleFormat=bidask&granularity=' + granularity + '&dailyAlignment=0&alignmentTimezone=America%2FNew_York', headers = { 'Authorization': 'Bearer '+ token })
...
def get_instrument_list ( account, token ):
	...
	# Send request to oanda and check for 200 response status code.
	response = requests.get('https://api-fxtrade.oanda.com/v1/instruments?accountId=' + str(account), headers = { 'Authorization': 'Bearer ' + token })
	status_code = response.status_code
...
def get_account ( account, token ):
	...
	# Send request to oanda and check for 200 response status code.
	response = requests.get('https://api-fxtrade.oanda.com/v1/accounts/' + str(account), headers = { 'Authorization': 'Bearer '+ token })
	status_code = response.status_code
...
def get_current_price ( pair, token ):
	...
	# Send request to oanda and check for 200 response status code.
	response = requests.get('https://api-fxtrade.oanda.com/v1/prices?instruments=' + pair, headers = { 'Authorization': 'Bearer '+ token })
	status_code = response.status_code
...
def create_order ( pair, account, token, side, units, stop):
	...
	response = requests.post('https://api-fxtrade.oanda.com/v1/accounts/' + str(account) + '/orders', data = payload , headers = { 'Authorization': 'Bearer '+ token })
	status_code = response.status_code
	if status_code != 200:
		return False
	else:
...
def get_position ( pair, account, token ):
	# Send request to oanda and check for 200 response status code.
	response = requests.get('https://api-fxtrade.oanda.com/v1/accounts/' + str(account) + '/positions/' + pair, headers = { 'Authorization': 'Bearer '+ token })
	status_code = response.status_code
...
def delete_position ( pair, account, token ):
	# Send request to oanda and check for 200 response status code.
	response = requests.delete('https://api-fxtrade.oanda.com/v1/accounts/' + str(account) + '/positions/' + pair, headers = { 'Authorization': 'Bearer '+ token })
	status_code = response.status_code
	if status_code != 200:
		return False
	else:
